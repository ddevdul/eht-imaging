...
def main():
    ...
    # sgrscat=True blurs the visibilities with
    # the Sgr A* scattering kernel for the 
    # appropriate image frequency ampcal and phasecal 
    # determine if gain variations and 
    # phase errors are included
    tint_sec = 5
    tadv_sec = 600
    tstart_hr = 0
    tstop_hr = 24
    ...
    R = np.array(((c,-s), (s, c)))
    coordsrot = np.array(
        [np.dot(R,[obs.data['u'][i], obs.data['v'][i]]) 
