...
def main():
    ...
    im.imvec *= 0.0
    im = im.add_crescent(
        1.0, 25.*ehtim.RADPERUAS,
        20.*ehtim.RADPERUAS,
        0, 0)
    im = im.blur_circ(5.0*ehtim.RADPERUAS)
    # Look at the image
    im.display()
    tint_sec = 5
    tadv_sec = 600
    ...
    out  = ehtim.imager_func(
        ...
        d1=d1, d2=d2, alpha_d1=alpha_d1,
        alpha_d2=alpha_d2, s1='simple',
        alpha_s1=10, ttype='nfft',
        maxit=100, show_updates=False,
        beam_size=res, norm_reg=True
        )

    for _ in range(5):
        out  = ehtim.imager_func(
            obs, out.blur_circ(res), gaussprior, flux,
            d1=d1, d2=d2, alpha_d1=alpha_d1,
            alpha_d2=alpha_d2, s1='simple',
            alpha_s1=10, ttype='nfft',
            maxit=300, show_updates=False,
            beam_size=res, norm_reg=True
            )
    ...
    for _ in range(5):
        alpha_A *= 2.0
        out2 = ehtim.imager_func(
            obs, out2.blur_circ(res/2.0),
            gaussprior, flux, d1=d1, d2=d2,
            alpha_d1=alpha_d1, alpha_d2=alpha_d2,
            s1='tv2', s2='lA', alpha_s1=10.0,
            alpha_s2=10.0, ttype='nfft', alpha_A=alpha_A,
            maxit=300, show_updates=False,
            beam_size=res, norm_reg=True
            )
    ...
    for _ in range(5):
        alpha_A *= 2.0
        out2 = ehtim.imager_func(
            obs, out2.blur_circ(res/2.0), gaussprior,
            flux, d1=d1, d2=d2, alpha_d1=alpha_d1,
            alpha_d2=alpha_d2, s1='tv2', s2='lA',
            alpha_s1=20.0, alpha_s2=20.0, ttype='nfft',
            alpha_A = alpha_A, maxit=500, show_updates=False,
            beam_size=res, norm_reg=True
        )
    out2.display()
