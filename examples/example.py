...
def main():
    ...
    im.display()
    # Observe the image
    # tint_sec is the integration time in seconds, 
    # and tadv_sec is the advance time between scans
    # tstart_hr is the GMST time of the start of the 
    # observation and tstop_hr is the GMST time of the end
    # bw_hz is the  bandwidth in Hz
    # sgrscat=True blurs the visibilities with the 
    # Sgr A* scattering kernel for the appropriate image frequency
    # ampcal and phasecal determine if gain 
    # variations and phase errors are included
    tint_sec = 5
    tadv_sec = 600
    tstart_hr = 0
    tstop_hr = 24
    ...
    # You can deblur the visibilities by 
    # dividing by the scattering kernel if necessary
    # obs = obs.deblur()
    ...
    # obs_sc = self_cal.self_cal(obs, out)
    ...
    # out = out_sc
    # ehtim.comp_plots.plotall_obs_im_compare(obs, out,'uvdist','amp', clist=['b','m'],conj=True)
