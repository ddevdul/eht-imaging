...
def main():
    ...
    obs345_nospec = im345_nospec.observe(
        eht, tint_sec, tadv_sec,
        tstart_hr, tstop_hr, bw_hz,
        sgrscat=False, ampcal=True,
        phasecal=True
    ...
    emptyprior = ehtim.image.make_square(obs230, npix, fov)
    ...
    flux230 = zbl
    imgr230  = ehtim.imager.Imager(
        obs230, gaussprior, gaussprior, flux230,
        data_term={'vis':100}, show_updates=False,
        reg_term={'tv2':1.e4,'l1':5}, maxit=200, ttype='nfft'
        )
    imgr230.make_image_I(show_updates=True)
    out230 = imgr230.out_last()
    flux345 = im345.total_flux()
    imgr345  = ehtim.imager.Imager(
        obs345, gaussprior, gaussprior, flux345,
        data_term={'vis':100}, show_updates=False,
        reg_term={'tv2':1}, maxit=200, ttype='nfft'
        )
    imgr345.make_image_I(show_updates=False)
    out345 = imgr345.out_last()
    ## Image both frequencies together without 
    # multifrequency imaging (no spectral index)
    imgr  = ehtim.imager.Imager(
        obslist_nospec, gaussprior, gaussprior, zbl,
        data_term={'vis':100}, show_updates=False,
        reg_term={'tv2':1.e4,'l1':5}, maxit=200, ttype='nfft'
        )
    ...
    imgr  = ehtim.imager.Imager(
        obslist, gaussprior, gaussprior, zbl,
        data_term={'vis':100}, show_updates=False,
        reg_term={'tv2':1.e4,'l1':5}, maxit=100, ttype='nfft'
        )
    imgr.make_image_I(mf=True,show_updates=False)
    out = imgr.out_last()
    for i in range(3): # blur and reimage
        out = out.blur_circ(15*ehtim.RADPERUAS)
        imgr.maxit_next=500
        imgr.init_next = out
        imgr.make_image_I(mf=True,show_updates=False)
        out = imgr.out_last()
