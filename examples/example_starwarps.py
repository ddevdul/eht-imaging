...
that generates a movie from a single night of observation. 
...
def main():
    ...
    # image parameters
    flux = 2.0
    fwhm = 50 * ehtim.RADPERUAS
    fov = 100 * ehtim.RADPERUAS
    NPIX = 30
    npixels = NPIX**2
    ...
    dirname = '../results'
    # load in the data
    obs = ehtim.obsdata.load_uvfits(obsname)
    # split the observations based upon the time
    obs_List = starwarps.splitObs(obs)
    ...
    imCov = []
    imCov.append(
        starwarps.gaussImgCovariance_2(meanImg[0],
        powerDropoff=2.0,
        frac=1./2.)
        )
    # make the covariance matrix that says how much
    #  variation there should be between frames in time 
    noiseCov_img = np.eye(npixels)*variance_img_diff
    # initialize the flowbasis and get the initTheta 
    # which says how to specify no motion for the specified flow basis
    init_x, init_y, flowbasis_x, flowbasis_y, initTheta = (
        starwarps.affineMotionBasis_noTranslation(meanImg[0])
        )
    ...
    expVal_t, expVal_t_t, expVal_tm1_t, loglikelihood, apxImgs = (
        starwarps.computeSuffStatistics(
    ...
    if SAVE:
        ...
        starwarps.movie(expVal_t, out = dirname + '/movie_nomotion.mp4')
    ...
    for iter in range(0, nIters+1):
        ...
        if iter==0 or reassign_apxImgs:
            apxImgs = False
        # solve for the sufficient statistics u
        # sing the StarWarps approach with the previous value 
        # the warp parameters in newTheta
        expVal_t, expVal_t_t, expVal_tm1_t, loglikelihood, apxImgs = (
            starwarps.computeSuffStatistics(
            meanImg, imCov, obs_List, noiseCov_img,
            newTheta, init_x, init_y, 
            flowbasis_x, flowbasis_y, initTheta,
            method=warp_method, measurement=measurement, 
            interiorPriors=interiorPriors,
            numLinIters=numLinIters,
            compute_expVal_tm1_t=apxImgs
            ))
        # save the negative log likelihood (nll), 
        # the value of the warp parameters (thetas) 
        ...
        if SAVE: 
            ...
            #save flow diagram
            plt.figure(), starwarps.plot_Flow(
                avgImg, thetas[iter], init_x, init_y,
                flowbasis_x, flowbasis_y, initTheta, step=1
                )
            plt.savefig(dirname + '/flow_' + str(iter) +  '.pdf')
            #save a movie
            starwarps.movie(
                expVal_t, dirname + '/movie_'
                + str(iter) + '.mp4'
                )
            # save out mat file with the information
            scipy.io.savemat(
                dirname + '/info_' + str(iter) + '.mat', 
                {'negll':negll,
                'thetas':thetas,
                'funeval':fevals}
                )
        ...
        if iter < nIters:
            result = scipy.optimize.minimize(
                starwarps.expnegloglikelihood, newTheta,
                args=(expVal_t, expVal_t_t, expVal_tm1_t,
                        meanImg, imCov, obs_List, noiseCov_img, 
                        init_x, init_y, flowbasis_x, flowbasis_y, 
                        initTheta, warp_method), 
                        method='L-BFGS-B',
                        jac=starwarps.deriv_expnegloglikelihood,
                        bounds=bnds, options=optdict
                        )
            newTheta = result.x
            feval = result.fun
